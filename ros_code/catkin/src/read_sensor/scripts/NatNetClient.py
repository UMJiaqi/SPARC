...
class NatNetClient:
    ...
    def __init__( self ):
        ...
        self.serverIPAddress = "192.168.1.102"

        # Change this value to the IP address of your local network interface
        self.localIPAddress = "192.168.1.103"
        ...
        # Set this to a callback method of your choice to receive Frame data
        ...
        self.__rigidBodies = dict()  # [id] = (pos, rot)
        self.__labeledMarkers = dict()  # [id] = (pos, size)
    ...
    def marker_info(self):
        ...
        rigidBodies = copy.deepcopy(self.__rigidBodies)
        labeledMarkers = copy.deepcopy(self.__labeledMarkers)
        self.__dataLock.release()
        return rigidBodies, labeledMarkers
    ...
    def __unpackMocapData( self, data ):
        ...
        if( ( self.__natNetStreamVersion[0] == 2 and self.__natNetStreamVersion[1] > 3 ) or self.__natNetStreamVersion[0] > 2 ):
            ...
            for i in range( 0, labeledMarkerCount ):
                ...
                self.__dataLock.acquire()
                self.__labeledMarkers[id] = (pos, size)
                self.__dataLock.release()
        ...
        if self.newFrameListener is not None:
            self.newFrameListener( frameNumber, markerSetCount, unlabeledMarkersCount, rigidBodyCount, skeletonCount,
                                  labeledMarkerCount, self.__labeledMarkers, timecode, timecodeSub, timestamp,
    ...
    def __dataThreadFunction( self, user_sock ):
        while not self._stop_flag:
            # Block for input
            try:
                user_sock.settimeout(1)
                data, addr = user_sock.recvfrom( 32768 ) # 32k byte buffer size
                if( len( data ) > 0 ):
    ...
    def run( self ):
        ...
        self.dataThread = Thread( target = self.__dataThreadFunction, args = (self.dataSocket, ))
        self.dataThread.start()

        # Create a separate thread for receiving command packets
        self.commandThread = Thread( target = self.__dataThreadFunction, args = (self.commandSocket, ))
